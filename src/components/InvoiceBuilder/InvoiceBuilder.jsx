...
const InvoiceBuilder = () => {
    ...
    const { downloadPDF, isGenerating, error } = usePDF();
    ...
    const handleDownload = () => {
        ...
        // In Phase 1, we just call the hook
        downloadPDF(invoiceData, totals, false); // isPro = false for Phase 1 free version
    };
            ...
            <div className="mx-auto max-w-[1300px] px-4 md:px-8">
                <div className="grid grid-cols-1 gap-8 lg:grid-cols-12 relative">
                    ...
                    <div className="lg:col-span-7 space-y-6">
                        ...
                        <div className="sticky bottom-6 z-40 mt-10 rounded-2xl bg-white border-2 border-brand p-4 shadow-xl flex items-center justify-between">
                            <div>
                                ...
                                <div className="text-xs text-muted">{t('pricing.free')} · PDF Watermarked</div>
                            </div>

                            <button
                                onClick={handleDownload}
                                disabled={isGenerating}
                                className={`rounded-xl bg-brand px-6 py-3 font-bold text-white shadow-lg shadow-brand/20 transition-all hover:bg-brand-dark hover:-translate-y-0.5 flex items-center gap-2 ${isGenerating ? 'opacity-70 cursor-wait' : ''}`}
                            >
                                {isGenerating ? (
                    ...
                    <div className="lg:col-span-5 hidden lg:block relative">
                        <PDFPreview invoiceData={invoiceData} totals={totals} />
                    </div>
